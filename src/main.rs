...
fn main() {
    ...
    let modules: Vec<Module> = args.module.unwrap_or_else(|| vec![Module::Param, Module::Quotes]);

    let mut pipeline = Pipeline::new(OS::Linux);
    for m in modules {
        match m {
            Module::Quotes  => { pipeline = pipeline.add(QuotesObfuscator); }
            Module::Hex     => { pipeline = pipeline.add(HexObfuscator); }
            Module::Param   => { pipeline = pipeline.add(ParamObfuscator); }
            Module::Reverse => { pipeline = pipeline.add(ReverseObfuscator); }
        }
    }
